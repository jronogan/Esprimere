...
import { fmt12h, formatDate } from "@/lib/functions";
...
function classStatus(c: TodayClassDTO) {
  if (!c.instructor)
    return {
      label: "No instructor",
      cls: "bg-[#FCEBEB] text-[#A32D2D] border-[#F09595]",
    };
  if (c.bookingCount >= c.maxPax)
    return {
      label: "Full",
      cls: "bg-[#FAEEDA] text-[#633806] border-[#EF9F27]",
    };
...
const thCls =
  "text-left text-[11px] font-medium text-gray-400 uppercase tracking-[0.3px] pb-2 border-b border-gray-100";
const tdCls =
  "py-2.5 border-b border-gray-100 last:border-0 text-[13px] text-gray-800 align-top";
  ...
  const overviewStats = [
    {
      ...
      val: stats.classesThisWeek,
      sub: "class bookings",
    },
    {
      label: "Bookings today",
      val: stats.bookingsToday,
      sub: "confirmed + pending",
    },
    {
      label: "Studio rentals",
      val: stats.rentalsThisMonth,
      sub: "this month",
    },
    {
  ...
  return (
    <div>
      ...
      <p className="text-[13px] text-gray-500 mt-0.5 mb-5">
        Overview for {studioName}
      </p>

      {/* Stat cards */}
      <div className="grid grid-cols-4 gap-2.5 mb-6">
        {overviewStats.map(({ label, val, sub }) => (
          <div
            key={label}
                ...
                return (
                  <tr key={c.id}>
                    ...
                    <td className={tdCls}>
                      <span
                        onClick={() =>
                          setManageSlot({ id: c.id, title: c.title })
                        }
      ...
      <div className="bg-white border border-gray-100 rounded-lg p-5">
        ...
        {upcomingRentals.length === 0 ? (
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr>
                {["Renter", "Room", "Date & time", "Paid", "Status"].map(
                  (h) => (
                    <th key={h} className={thCls + " pr-3"}>
                      {h}
                    </th>
            ...
            <tbody>
              {upcomingRentals.map((r) => (
                <tr key={r.id}>
                  ...
                  <td className={tdCls + " pr-3 text-gray-500"}>
                    {formatDate(r.date)} · {fmt12h(r.startTime)}–
                    {fmt12h(r.endTime)}
                  </td>
                  <td className={tdCls + " pr-3 text-gray-500"}>
                  ...
                  <td className={tdCls}>
                    <span
                      className={`text-[11px] px-2 py-0.5 rounded-full border ${rentalStatusCls(r.status)}`}
                    >
