...
function statusBadge(status: string) {
  ...
  if (status === "CANCELLED") return "bg-[#FCEBEB] text-[#A32D2D] border-[#F09595]";
  if (status === "PENDING") return "bg-[#FAEEDA] text-[#633806] border-[#EF9F27]";
  return "bg-gray-100 text-gray-500 border-gray-200";
}

const thCls = "text-left text-[11px] font-medium text-gray-400 uppercase tracking-[0.3px] pb-2 border-b border-gray-100 pr-3 py-3";
...
export default function AllBookingsTab({ bookings }: Props) {
  ...
  return (
    <div>
      ...
      <p className="text-[13px] text-gray-500 mt-0.5 mb-4">Class and studio rental bookings across all rooms</p>
      ...
      <div className="bg-white border border-gray-100 rounded-lg overflow-hidden">
        {filtered.length === 0 ? (
          <p className="text-[13px] text-gray-400 p-5">No {filter === "all" ? "" : filter} bookings found.</p>
        ) : (
          <table className="w-full border-collapse">
            <thead>
              <tr className="px-5">
                {["Name", "Type", "Date & time", "Room", "Amount", "Status"].map((h) => (
                  <th key={h} className={`${thCls} first:pl-5`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map((b) => (
                <tr key={b.id}>
                  ...
                  <td className={tdCls}>
                    <span className={`text-[11px] px-2 py-0.5 rounded-full border ${statusBadge(b.status)}`}>
